...
from .views import applyDiscount
import datetime
...
class DiscountsTest(TestCase):
    ...
    def test_Discounts_Serializer(self):
        discount1 = Discounts.objects.get(name = 'test')
        serialized = DiscountsSerializer(discount1)
        self.assertTrue(serialized.is_valid())

    def test_Apply_Discount(self):
        discount1 = Discounts.objects.get(name = 'test')
        applyDiscount(discount_code= discount1.code)
        discount2 = Discounts.objects.get(name = 'test')
        self.assertEqual(discount2.quantity_available, discount1.quantity_available - 1)
        self.assertEqual(discount2.quantity_sold, discount1.quantity_sold + 1)
