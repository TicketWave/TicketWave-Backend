...
class TicketTestCase(TestCase):
    def setUp(self):
        Ticket.objects.create(
            name="Amir", category="Testing", subcategory="subtest", amount=10
        )
        Ticket.objects.create(
            name="youssef", category="Testing", subcategory="subtest", amount=1000
        )
    ...
    def test_TicketEndpointCreate(self):  # testing create
    ...
    def test_TicketEndpointGetAll(self):  # testing get all
        url = "127.0.0.1:8000/tickets/get"
        response = requests.get(url=url)
        self.assertEqual(response.status_code, 200)

    def test_TicketEndpointUpdate(self):  # testing update
